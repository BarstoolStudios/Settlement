...
//==============================================================================
float TerrainUtil::perlin(int size, float x, float y, int NOISE[Terrain::NOISE_SIZE][Terrain::NOISE_SIZE]) {
	int nx, ny;
	if (x < 0)
	...
	Vector2f g00 = gradientSet[random(p00[0], p00[1], NOISE) % 8];
	Vector2f g10 = gradientSet[random(p10[0], p10[1], NOISE) % 8];
	Vector2f g01 = gradientSet[random(p01[0], p01[1], NOISE) % 8];
	Vector2f g11 = gradientSet[random(p11[0], p11[1], NOISE) % 8];
...
//==============================================================================
float TerrainUtil::octivate(int nOctives, int size, float x, float y, int NOISE[Terrain::NOISE_SIZE][Terrain::NOISE_SIZE]) {
	float oct = 0;
	float amplitude = 0.5f;
	int freq = 2;
	for (int i = 0; i < nOctives; i++) {
		oct += (perlin(size / freq, x, y, NOISE) * amplitude);
		amplitude *= 0.5f;
		freq *= 2;
	}
	return oct;
...
//==============================================================================
int TerrainUtil::random(int x, int y, int NOISE[Terrain::NOISE_SIZE][Terrain::NOISE_SIZE]) {
	return NOISE[std::abs(hashX(x + 1301081)) % 256][std::abs(hashY(y + 2751557)) % 256];
}

//==============================================================================
// 5th Order Hermite Function (6x^5 - 15x^4 + 10x^3) - (6y^5 - 15y^4 + 10y^3)
//==============================================================================
